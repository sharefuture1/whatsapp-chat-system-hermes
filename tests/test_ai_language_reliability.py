...
from test_translations_dispatcher import (
    _FakeRewrite, _FakeWorker, _SessionTracker, _make_batch, _make_dispatcher, _seed,
...
def test_ai_settings_save_refreshes_cached_rewriter_and_worker_manager(tmp_path, monkeypatch):
    ...
    config = SimpleNamespace(paths=app.state.runtime.paths, ai_settings=app.state.runtime.ai_settings)
    ...
    with TestClient(app) as client:
        ...
        response = client.put('/api/v1/ai/settings', headers={'x-session-token': token}, json={
            'api_key': 'unit-test-runtime-secret', 'default_model': 'runtime-test-model',
        })
        ...
        assert rewriter.ai_service.resolve_model(contact_model='contact-model').model == 'contact-model'
...
def test_empty_or_unchanged_translation_is_not_completed(factory, bad_output):
    ...
    batch_id = _make_batch(factory, anchor_id=anchor, account_id=account_id, conversation_id=conversation_id)
    tracker = _SessionTracker(factory)
    worker = _FakeWorker(tracker, batch_payload={'items': [{
        'message_id': messages[0], 'source_lang': 'English', 'zh': bad_output,
...
def test_translation_requests_reuse_active_batch_and_reject_wrong_target(tmp_path, monkeypatch):
    app, _ = standalone_app(tmp_path, monkeypatch)
    ...
    with TestClient(app) as client:
        ...
        invalid = client.post(path, json={'anchor_message_id': anchor, 'target_lang': 'en'})
...
def test_whitespace_source_hash_is_stable_across_batches(factory):
    ...
    tracker = _SessionTracker(factory)
    worker = _FakeWorker(tracker, batch_payload={'items': [{
        'message_id': messages[0], 'source_lang': 'English', 'zh': '你好',
    }]})
    dispatcher = _make_dispatcher(factory, tracker, worker)
    for _ in range(2):
        _make_batch(factory, anchor_id=anchor, account_id=account_id, conversation_id=conversation_id)
        dispatcher.run_once()
