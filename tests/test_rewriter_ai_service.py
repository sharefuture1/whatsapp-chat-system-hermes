...
def test_rewriter_smart_translate_and_auto_translate_use_ai_service():
    ...
    translated = rewriter.translate_only({'id': 'u', 'name': 'User'}, '你好', 'Preferred language: Thai')
    auto = rewriter.translate_to_zh('ສະບາຍດີ', 'Lao')

    assert smart.message == 'สวัสดี'
    assert translated.message == 'สวัสดี'
    assert auto == '你好'
    assert [call['model'] for call in provider.calls] == ['contact-model', 'account-model', 'account-model']


def test_rewriter_default_ai_service_injects_existing_logger_for_audit():
    events = []
    rewriter = Rewriter(DummyConfig(), lambda event, **fields: events.append((event, fields)))
...
def test_rewriter_provider_failure_preserves_fallback_and_error_metadata():
    ...
    result = rewriter.rewrite({'id': 'u', 'name': 'User'}, '你好', 'Preferred language: Thai')
    translated = rewriter.translate_only({'id': 'u', 'name': 'User'}, '你好', 'Preferred language: Thai')
