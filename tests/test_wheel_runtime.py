...
def test_installed_package_finds_migrations_in_service_working_directory(tmp_path, monkeypatch):
    expected = api_module._current_alembic_head()
    project_root = Path(__file__).resolve().parents[1]
    monkeypatch.setattr(api_module, "__file__", str(tmp_path / "site-packages" / "whatsapp_chat_system" / "standalone_api.py"))
