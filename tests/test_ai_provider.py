...
def test_provider_maps_http_errors_and_retries_only_retryable(status, code, retryable, expected_calls):
...
def test_settings_normalize_and_bound_environment_values():
    ...
    invalid_port = AISettings.from_env({'WENDING_AI_BASE_URL': 'https://example.com:99999/v1'})
...
def test_provider_maps_invalid_timeout_to_configuration_error():
    provider = WendingAIProvider(AISettings(api_key='unit-test-secret', timeout_seconds=0))
