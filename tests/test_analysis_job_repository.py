...
def _engine(tmp_path):
    engine = create_engine(
        f"sqlite:///{tmp_path / 'jobs.db'}", connect_args={"timeout": 5}
    )
...
def _enqueue(
    repo, account_id, key="k", input_hash="a" * 64, available_at=None, **kwargs
):
    return repo.enqueue(
        account_id,
        "profile_refresh",
...
def test_enqueue_is_scoped_idempotent_atomic_and_session_recovers(tmp_path):
    ...
    with Session(engine) as session:
        ...
        assert (
            _enqueue(repo, a1.id, contact_id=c1.id, conversation_id=v1.id).id
            == first.id
        )
        ...
        with pytest.raises(JobScopeViolation):
            _enqueue(
                ...
                a1.id,
                key="bad-parent",
                parent_job_id=_enqueue(repo, a2.id, key="p").id,
            )
...
def test_claim_order_account_filter_and_two_sessions_cannot_claim_same_job(tmp_path):
    ...
    with factory() as setup:
        a1, _, _ = _scope(setup, "1")
        a2, _, _ = _scope(setup, "2")
        repo = AnalysisJobRepository(setup)
        _enqueue(
            repo, a1.id, key="low", priority=1, available_at=now - timedelta(minutes=3)
        )
        high_late = _enqueue(
            repo,
            a1.id,
    ...
    with factory() as s1, factory() as s2:
        ...
        assert (
            one.id == high_early.id
            and one.status == "claimed"
            and one.attempts == 1
            and one.version == 2
        )
        s1.commit()
        ...
        s2.commit()
    with factory() as s1, factory() as s2:
        claimed = AnalysisJobRepository(s1).claim_next(
            "w3", 30, account_id=a2.id, now=now
        )
        s1.commit()
        assert claimed.id == other.id
        assert (
            AnalysisJobRepository(s2).claim_next("w4", 30, account_id=a2.id, now=now)
            is None
        )
...
def test_start_heartbeat_complete_enforce_owner_version_lease_and_input_hash(tmp_path):
    ...
    now = datetime(2026, 7, 11, 12, tzinfo=UTC)
    with Session(engine) as session:
        account, _, _ = _scope(session)
        repo = AnalysisJobRepository(session)
        job = _enqueue(repo, account.id, available_at=now)
        job = repo.claim_next("worker", 10, now=now)
        with pytest.raises(JobConflict):
            repo.start(account.id, job.id, "other", job.version, now=now)
        ...
        assert (
            ...
            and done.lease_owner is None
            and done.version == heartbeat_version + 1
        )
        with pytest.raises(JobConflict):
            repo.complete(account.id, done.id, "worker", heartbeat_version, "a" * 64)
...
def test_expired_lease_rejects_worker_and_recovery_invalidates_old_version(tmp_path):
    ...
    with Session(engine) as session:
        account, _, _ = _scope(session)
        repo = AnalysisJobRepository(session)
        _enqueue(repo, account.id, available_at=now)
        ...
        assert (
            ...
            and current.version == old_version + 1
            and current.lease_owner is None
        )
        with pytest.raises(JobConflict):
            repo.complete(account.id, claimed.id, "old", old_version, "a" * 64)
...
def test_fail_retries_then_dead_and_max_attempts_are_never_reclaimed(tmp_path):
    ...
    with Session(engine) as session:
        account, _, _ = _scope(session)
        repo = AnalysisJobRepository(session)
        _enqueue(repo, account.id, max_attempts=2, available_at=now)
        first = repo.claim_next("w", 30, now=now)
        retried = repo.fail(
            account.id, first.id, "w", first.version, "provider_timeout", 5, now=now
        )
        assert retried.status == "retry" and retried.available_at.replace(
            tzinfo=UTC
        ...
        assert (
            dead.status == "dead"
            and repo.claim_next("w", 30, now=now + timedelta(days=1)) is None
        )
...
def test_recover_limit_dead_normalization_cancel_scope_and_terminal_conflict(tmp_path):
    ...
    now = datetime(2026, 7, 11, 12, tzinfo=UTC)
    with Session(engine) as session:
        a1, _, _ = _scope(session, "1")
        a2, _, _ = _scope(session, "2")
        repo = AnalysisJobRepository(session)
        j1 = _enqueue(repo, a1.id, key="one", max_attempts=1, available_at=now)
        j2 = _enqueue(repo, a1.id, key="two", max_attempts=1, available_at=now)
        repo.claim_next("w", 1, account_id=a1.id, now=now)
        repo.claim_next("w", 1, account_id=a1.id, now=now)
        ...
        pending = _enqueue(repo, a1.id, key="cancel", available_at=now)
        with pytest.raises(JobScopeViolation):
            repo.cancel(a2.id, pending.id, pending.version)
        cancelled = repo.cancel(a1.id, pending.id, pending.version)
        assert cancelled.status == "cancelled"
        with pytest.raises(JobConflict):
            repo.cancel(a1.id, cancelled.id, cancelled.version)


def test_postgres_claim_query_compiles_skip_locked():
    now = datetime.now(UTC)
    sql = str(
        build_postgres_claim_query(None, now, 100, "2.5", 7).compile(
...
def test_postgres_recovery_query_compiles_skip_locked_and_scope():
    sql = str(
        build_postgres_recovery_query(datetime.now(UTC), 10, "account").compile(
            dialect=postgresql.dialect(), compile_kwargs={"literal_binds": True}
        )
    ).upper()
...
def test_validation_idempotency_backpressure_and_progress(tmp_path):
    engine = _engine(tmp_path)
    with Session(engine) as session:
        account, contact, conversation = _scope(session)
        repo = AnalysisJobRepository(session)
        for kwargs in (
            {"input_hash": "x"},
            {"job_type": ""},
            {"priority": -1},
            {"budget_tokens": -1},
            {"progress_total": 1, "progress_completed": 2},
        ):
            args = dict(
                account_id=account.id,
...
def test_parent_cancel_propagates_only_unstarted_and_rejects_worker_result(tmp_path):
    ...
    with Session(engine) as session:
        account, _, _ = _scope(session)
        repo = AnalysisJobRepository(session)
        parent = _enqueue(repo, account.id, key="parent", priority=10, available_at=now)
        pending = _enqueue(
            repo, account.id, key="pending", parent_job_id=parent.id, available_at=now
        )
        running = _enqueue(
            repo,
            account.id,
...
def test_exact_expiry_rejected_and_recoverable(tmp_path):
    ...
    with Session(engine) as session:
        ...
        repo = AnalysisJobRepository(session)
        _enqueue(repo, account.id, available_at=now)
        job = repo.claim_next("w", 1, now=now)
        with pytest.raises(JobConflict):
            repo.start(
                account.id, job.id, "w", job.version, now=now + timedelta(seconds=1)
            )
        assert (
            repo.recover_expired_leases(
                now + timedelta(seconds=1), account_id=account.id
            )
            == 1
        )
...
def test_committed_claim_wrapper_is_immediately_visible(tmp_path):
    ...
    with factory() as session:
        account, _, _ = _scope(session)
        _enqueue(AnalysisJobRepository(session), account.id, available_at=now)
        session.commit()
...
def test_global_claim_skips_accounts_at_active_limit_and_returns_none_when_all_full(
    tmp_path,
):
    engine = _engine(tmp_path)
    now = datetime(2026, 7, 11, 12, tzinfo=UTC)
    with Session(engine) as session:
        full, _, _ = _scope(session, "full")
        free, _, _ = _scope(session, "free")
        repo = AnalysisJobRepository(session)
        _enqueue(repo, full.id, key="active", priority=100, available_at=now)
        active = repo.claim_next("busy", 30, account_id=full.id, now=now)
        _enqueue(repo, full.id, key="blocked", priority=99, available_at=now)
        candidate = _enqueue(
            repo, free.id, key="candidate", priority=1, available_at=now
        )
        claimed = repo.claim_next("global", 30, now=now, max_active_per_account=1)
        assert claimed.id == candidate.id
        _enqueue(repo, free.id, key="blocked-too", priority=0, available_at=now)
        assert (
            repo.claim_next("global-2", 30, now=now, max_active_per_account=1) is None
        )
...
def test_cancelled_parent_blocks_enqueue_and_recovery_cancels_leased_child(tmp_path):
    ...
    with Session(engine) as session:
        ...
        repo = AnalysisJobRepository(session)
        parent = _enqueue(
            repo, account.id, key="parent", priority=100, available_at=now
        )
        leased = _enqueue(
            repo,
            account.id,
        ...
        assert (
            repo.claim_next(
                "again", 30, account_id=account.id, now=now + timedelta(seconds=3)
            )
            is None
        )
...
def test_parent_lock_query_compiles_for_update():
    sql = str(
        build_parent_lock_query("parent-id", "account-id").compile(
            dialect=postgresql.dialect(), compile_kwargs={"literal_binds": True}
        )
    ).upper()
