...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "Hello, how can I book?",
            "Hello, which day would you like to book?",
            "English/Latin",
        ),
    ],
)
def test_worker_pins_language_to_inbound_message(tmp_path, text, reply, language):
    ...
    worker = AutoReplyWorker(
        factory, _SettingsManager(), provider_factory=lambda _: Provider()
    )
    worker.run_once()
        ...
        def chat(self, **kwargs):
            if opt_out:
                with factory() as session:
                    ...
                    session.commit()
            return AIResult(
                "สวัสดีครับ" if opt_out else "你好", kwargs["model"], None, {}, 1
            )
