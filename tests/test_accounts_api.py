...
def create_account(client, name='Sales'):
    response = client.post('/api/v1/accounts', json={'name': name, 'auto_reply_mode': 'suggest'})
...
def test_qr_only_returns_when_account_is_qr_pending(api):
    ...
    with factory() as db:
        row = db.get(__import__('whatsapp_chat_system.db.models', fromlist=['WhatsAppAccount']).WhatsAppAccount, account['id'])
...
def test_delete_requires_confirm_name_and_keeps_delete_session_independent(api):
    ...
    wrong = client.request(
        'DELETE', f"/api/v1/accounts/{account['id']}", json={'confirm_name': 'wrong', 'delete_session': True}
    )
    assert wrong.status_code == 409
    assert bridge.calls[-1][0] == 'create'

    deleted = client.request(
        'DELETE', f"/api/v1/accounts/{account['id']}", json={'confirm_name': 'Sales', 'delete_session': False}
    )
...
def test_bridge_unavailable_returns_structured_retryable_error(api):
    ...
    def unavailable(account_id):
        raise BridgeError('bridge_unavailable', 'offline', retryable=True, status_code=503)
...
def test_disable_bridge_failure_keeps_account_enabled(api):
    ...
    response = client.patch(f"/api/v1/accounts/{account['id']}", json={'enabled': False})
