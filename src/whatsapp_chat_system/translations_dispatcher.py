...
class TranslationDispatcher:
    ...
    def run_once(self) -> bool:
        ...
        with self.session_factory() as session:
            batch = session.scalar(
                ...
                .where(TranslationBatch.status == "pending")
            ...
            try:
                ...
                logger.exception(
                    "Translation batch failed", extra={"batch_id": batch.id}
                )
    ...
    def _process_batch(self, session: Session, batch: TranslationBatch) -> None:
        ...
        if pending_items:
            ...
            for item in pending_items:
                ...
                if result is None:
                    fallback = worker.translate_to_zh_result(
                        item["text"], item["source_lang"]
                    )
                ...
                if result.get("error"):
                    self._upsert_translation(
                        ...
                        batch,
                        source_lang=str(
                            result.get("source_lang") or item["source_lang"]
                        ),
                        translated_text=None,
                        status="failed",
                        error_code="translate_failed",
    ...
    def _translate_window(
        self, worker: Rewriter, pending_items: list[dict[str, Any]]
