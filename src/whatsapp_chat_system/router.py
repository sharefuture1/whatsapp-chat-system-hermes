...
class AdminRouter:
    ...
    def load_state(self) -> RouterState:
        payload = load_json(
            ...
            {'last_message_id': 0, 'handled_message_ids': [], 'last_command': None, 'last_result': None},
        )
        return RouterState(
            last_message_id=int(payload.get('last_message_id', 0)),
            handled_message_ids=[int(x) for x in payload.get('handled_message_ids', [])],
    ...
    def prepare_reply(self, target_text: str, message: str, mode: str = 'direct') -> dict[str, Any]:
    ...
    def send_prepared_reply(self, target: dict[str, Any], rewrite: RewriteResult, source_text: str, mode: str) -> dict[str, Any]:
        send_result = self.messenger.send_whatsapp(str(target.get('id')), rewrite.message, json_output=True)
    ...
    def run(self) -> int:
        ...
        rows = self.db.fetch_admin_messages(state.last_message_id, self.config.admin_ids)
        ...
        for row in rows:
            ...
            if text in INFO_COMMANDS:
                msg = self.format_alias_list(aliases) if text in {'联系人列表', '联系人编号', '别名列表', 'alias', 'aliases'} else self.format_status(state, aliases)
            ...
            if incomplete_target:
                self.messenger.send_admin_text(f'目标已识别为 {incomplete_target}，请继续发送完整格式：发给 {incomplete_target}：内容', kind='reply_ack')
                handled.add(rid)
                self.logger.log('incomplete_command', message_id=rid, target=incomplete_target)
                continue
            ...
            if not target:
                self.messenger.send_admin_text(f'未找到目标用户：{parsed["target"]}。请检查名字、ID，或先发送“联系人编号”查看列表。', kind='reply_ack')
                handled.add(rid)
                state.last_result = {'command': text, 'status': 'target_not_found'}
                self.logger.log('target_not_found', message_id=rid, target=parsed['target'])
                continue

            memory_md = self.load_user_memory(self.config, str(target.get('id') or ''))
            rewrite = self.rewriter.rewrite(target, parsed['message'], memory_md)
            result = self.send_prepared_reply(target, rewrite, parsed['message'], mode='smart')
            ...
            if send_ok:
                ...
                self.logger.log('send_success', message_id=rid, target=target.get('id'), alias=target.get('alias'), language=rewrite.language, final_text=rewrite.message)
            else:
                ack = (
                ...
                self.logger.log('send_failed', message_id=rid, target=target.get('id'), stdout=result['stdout'], stderr=result['stderr'])
            handled.add(rid)

        state.last_message_id = max_id
        state.handled_message_ids = sorted(handled)[-500:]
        self.save_state(state)
        self.logger.log('scan_end', last_message_id=max_id, handled_count=len(state.handled_message_ids))
        return 0
