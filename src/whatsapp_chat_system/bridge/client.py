...
class BridgeError(Exception):
    def __init__(
        self, code: str, message: str, *, retryable: bool, status_code: int = 502
    ) -> None:
...
class BridgeClient:
    def __init__(
        ...
        *,
        ...
        if not allow_external_host and parsed.hostname not in {
            "127.0.0.1",
            "localhost",
            "::1",
        }:
    ...
    def _request(
        ...
        *,
        ...
        try:
            ...
            raise BridgeError(
                "bridge_timeout", "Bridge request timed out", retryable=True
            ) from exc
        except requests.ConnectionError as exc:
            raise BridgeError(
                "bridge_unavailable", "Bridge is unavailable", retryable=True
            ) from exc
        except requests.RequestException as exc:
            raise BridgeError(
                "bridge_request_failed", "Bridge request failed", retryable=True
            ) from exc
        ...
        if response.status_code == 410 and remote_code == "qr_expired":
            raise BridgeError(
                "qr_expired",
                message or "QR code expired",
                retryable=True,
                status_code=410,
            )
        if response.status_code == 401:
            raise BridgeError(
                "bridge_unauthorized", message, retryable=False, status_code=502
            )
        if response.status_code == 503:
            raise BridgeError(
                "bridge_unavailable", message, retryable=True, status_code=503
            )
        raise BridgeError(
    ...
    def create_account(self, account_id: str, session_ref: str) -> dict[str, Any]:
        return self._request(
            "POST",
            "/accounts",
            json={"account_id": account_id, "session_ref": session_ref},
        )
    ...
    def connect(self, account_id: str) -> dict[str, Any]:
        return self._request(
            "POST",
            f"/accounts/{self._account_path(account_id)}/connect",
            expected={200, 202},
        )

    def status(self, account_id: str) -> dict[str, Any]:
        return self._request(
            "GET", f"/accounts/{self._account_path(account_id)}/status"
        )
    ...
    def logout(self, account_id: str) -> dict[str, Any]:
        return self._request(
            "POST", f"/accounts/{self._account_path(account_id)}/logout"
        )

    def stop(self, account_id: str) -> dict[str, Any]:
        return self._request(
            "POST",
            f"/accounts/{self._account_path(account_id)}/stop",
            expected={200, 202},
        )
    ...
    def delete(
        ...
        return self._request(
            "DELETE", f"/accounts/{self._account_path(account_id)}{suffix}"
        )
