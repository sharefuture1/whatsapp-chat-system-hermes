...
class RuntimeAISettings(Protocol):
    """Structural contract implemented by the runtime settings manager."""
...
def _http_error(status_code: int, request_id: str | None) -> AIProviderError:
    if status_code == 401:
        return AIProviderError(
            ...
            message="Wending AI authentication failed",
            retryable=False,
    ...
    if status_code == 429:
        return AIProviderError(
            code="rate_limited",
            message="Wending AI rate limit exceeded",
            retryable=True,
            status_code=status_code,
            request_id=request_id,
        )
    if status_code >= 500:
        return AIProviderError(
            code="upstream_error",
            message="Wending AI upstream service failed",
            retryable=True,
            status_code=status_code,
            request_id=request_id,
        )
    return AIProviderError(
        code="request_error",
        message=f"Wending AI request failed with HTTP {status_code}",
        retryable=False,
        status_code=status_code,
        request_id=request_id,
    )
