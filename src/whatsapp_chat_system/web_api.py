            ...
            if api_key is not None:  # 空字符串=清除，非空=加密存储
                if api_key.strip():
                    ...
                    with self._lock:
                        ...
                        self._db_key_hint = row.api_key_hint
                        self._override_api_key = (
                            None  # 清除内存覆盖，改为从 DB 密文解密
                        )
                else:
...
def _display_user_name(
    origin: dict[str, Any], user_id: str, session: dict[str, Any]
) -> str:
...
def _name_for_user(
    config: AppConfig, user_id: str, session_ids: list[str], fallback: str = ""
) -> str:
...
def _hide_messages_enabled(config: AppConfig) -> bool:
    return bool(
        config.web_settings.get("message_ops", {}).get("hide_messages_enabled", False)
    )
...
def _hidden_message_ids(config: AppConfig) -> set[int]:
    ...
    return {
        int(x)
        for x in (config.web_settings.get("hidden_message_ids") or [])
...
def _compute_conversation_summaries(
    ...
    chat_ops = (
        config.web_settings.get("chat_ops")
        if isinstance(config.web_settings.get("chat_ops"), dict)
        else {}
    )
    ...
    for row in rows:
        ...
        user_name = _name_for_user(
            config, user_id, session_ids, str(row["title"] or "")
        )
        ...
        results.append(
            {
                ...
                "last_message_translated": _maybe_translate_for_user(
                    config, user_id, last_text
                ),
...
def _infer_priority(memory_markdown: str) -> str:
    ...
    return (
        "high"
        if "不舒服" in memory_markdown or "emotionally vulnerable" in memory_markdown
        else "normal"
    )
...
def _update_all_cached_translations(
    config: AppConfig, source: str, corrected: str
) -> None:
    """
    When a translation is corrected, update it in every user's message cache.
    Iterates all .translations__*.json files under memory_dir and replaces
    entries whose source_text matches the given source.
    """
    import glob
    from .translations import load_translations, put_translation

    mem_dir = Path(config.paths.memory_dir)
    pattern = str(mem_dir / ".translations__*.json")
    for path_str in glob.glob(pattern):
        try:
            path = Path(path_str)
            # user_id is between the double underscores: .translations__userId.json
            parts = path.stem.split("__", 1)
            if len(parts) < 2:
                continue
            user_id = parts[1]
            data = load_translations(mem_dir, user_id)
            changed = False
            for mid, item in data.get("items", {}).items():
                src = str(item.get("source_text") or "")
                if src.strip().lower() == source.strip().lower():
                    item["zh"] = corrected
                    changed = True
            if changed:
                # Write back all updated items via put_translation
                for mid, item_data in data.get("items", {}).items():
                    put_translation(mem_dir, user_id, mid, item_data)
        except Exception:
            pass  # fail-soft: one bad user file shouldn't break correction


def _ensure_message_translations(
    config: AppConfig, user_id: str, items: list[dict[str, Any]]
) -> None:
    ...
    cached = load_many(
        ...
        user_id,
        [m["message_id"] for m in items if m.get("message_id") is not None],
    )
...
def _dashboard_stats(
    conversations: list[dict[str, Any]], config: AppConfig
) -> dict[str, Any]:
...
def _check_login_rate_limit(
    ...
    attempts = [
        float(ts)
        for ts in login_attempts.get(ip, [])
    ...
    return [
        {
            ...
            "category": "chat",
            "login_type": "qr",
            "setup_command": "hermes -p <profile> whatsapp",
        },
        {
            "platform": "telegram",
            "label": "Telegram",
            "category": "chat",
            "login_type": "manual",
            "setup_command": "hermes -p <profile> telegram",
        },
        {
            "platform": "slack",
            "label": "Slack",
            "category": "team",
            "login_type": "manual",
            "setup_command": "hermes -p <profile> slack",
        },
        {
            "platform": "discord",
            "label": "Discord",
            "category": "community",
            "login_type": "manual",
            "setup_command": "hermes -p <profile> discord",
        },
    ]
...
def _sanitize_workspace_input(
    ...
    raw_profile = str(
        ...
        or payload.get("id")
        or workspace_id
        or f"{platform}-workspace"
    ).strip()
    profile = _slugify_workspace_value(raw_profile)
    workspace_key = _slugify_workspace_value(
        str(workspace_id or payload.get("id") or profile)
    )
    label = str(payload.get("label") or workspace_key).strip() or workspace_key
    profile_path = str(
        payload.get("profile_path") or f"/root/.hermes/profiles/{profile}"
    ).strip()
    return {
        "id": workspace_key,
        "label": label,
...
def _workspace_status(config: AppConfig) -> list[dict[str, Any]]:
    ...
    for raw_item in _workspace_entries(config):
        item = _sanitize_workspace_input(
            raw_item, workspace_id=str(raw_item.get("id") or "")
        )
        ...
        if exists:
            ...
            if running:
                status = "running"
        connect_command = str(meta.get("setup_command") or "").replace(
            "<profile>", profile
        )
        out.append(
            {
...
def build_app(
    ...
    *,
    ...
    resolved_account_factory = account_session_factory or create_session_factory(
        create_engine()
    )
    ...
    app.include_router(
        create_accounts_router(resolved_account_factory, resolved_account_bridge)
    )
    app.include_router(
    ...
    def search(q: str = "", limit: int = 30) -> dict[str, Any]:
        ...
        for row in db.search_messages(q, limit * 2, config.admin_ids):
            ...
            end_idx = (
                min(len(content), idx + len(q) + 30)
                if idx >= 0
                else min(len(content), 80)
            )
    ...
    def conversation_new_messages(
        user_id: str, after_id: int = 0, limit: int = 100
    ) -> dict[str, Any]:
        ...
        messages = _attach_translations(
            config, user_id, messages, generate_missing=False
        )
        return {
            "user_id": user_id,
            "messages": messages,
            "count": len(messages),
            "next_after_id": max(
                (int(m["message_id"]) for m in messages), default=after_id
            ),
            "max_message_id": max(
                (int(m["message_id"]) for m in messages), default=after_id
            ),
    ...
    def conversation_detail(
        user_id: str, page: int = 1, page_size: int = 80
    ) -> dict[str, Any]:
        page = max(1, page)
        page_size = max(1, min(500, page_size))
        ...
        language_hint = sidecar.get("preferred_language") or (
            "Lao"
            if "Preferred language: Lao" in memory_text
            else ("Thai" if "Preferred language: Thai" in memory_text else "Unknown")
        )
        auto_translate = bool(
            config.web_settings.get("message_ops", {}).get("auto_translate", True)
        )
    ...
    def reply(request: ReplyRequest) -> Any:
        ...
        try:
            prepared = router.prepare_reply(
                request.target, request.message, request.mode
            )
        ...
        result = router.send_prepared_reply(
            prepared["target"], prepared["rewrite"], request.message, request.mode
        )
        if result.get("success") is not True:
            logger.warning(
                "Message delivery failed for target=%s: %s",
                prepared["target"],
        ...
        result["local_message_id"] = db.append_assistant_message(
            target_id,
            str(
                (result.get("rewrite") or {}).get("message")
                or prepared["rewrite"].message
            ),
    ...
    def list_workspaces() -> dict[str, Any]:
        return {
            "items": _workspace_status(config),
    ...
    def delete_workspace(workspace_id: str) -> dict[str, Any]:
        items = _workspace_entries(config)
        remaining = [
            item for item in items if str(item.get("id") or "") != workspace_id
        ]
    ...
    def translate_message(
        message_id: str, body: dict[str, Any] | None = None
    ) -> dict[str, Any]:
        body = body or {}
        ...
        if not _auto_translate_enabled(config):
            raise HTTPException(
                status_code=409,
                detail={
                    "code": "auto_translate_disabled",
                    "message": "Auto translate plugin or message setting is disabled",
                },
            )
        public_message_id: int | str = (
    ...
    @app.post("/api/translations/correct")
    def correct_translation(body: dict[str, Any]) -> dict[str, Any]:
        """
        人工纠正一条翻译结果，写入记忆库并更新同句子的所有历史缓存。
        body: { source, source_lang, corrected_translation }
        """
        source = str(body.get("source") or "").strip()
        source_lang = str(body.get("source_lang") or "").strip()
        corrected = str(body.get("corrected_translation") or "").strip()
        if not source or not source_lang or not corrected:
            raise HTTPException(
                status_code=400,
                detail="source, source_lang, corrected_translation required",
            )
        worker = _translation_worker(config)
        # 1. 写入/更新记忆库，标记为 corrected=True
        worker.translation_memory.review_update(source, corrected, source_lang)
        # 2. 更新所有用户目录下同句子的 message 缓存翻译
        _update_all_cached_translations(config, source, corrected)
        return {"success": True, "source": source, "translation": corrected}
    ...
    def hide_messages(payload: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            "success": True,
            "hidden_message_ids": config.web_settings["hidden_message_ids"],
    ...
    def list_schedule() -> dict[str, Any]:
        ...
        items = sorted(
            ...
            key=lambda i: float(i.get("run_at") or 0),
        )
        return {"items": items}
    ...
    def list_broadcast() -> dict[str, Any]:
        ...
        items = sorted(
            [i for i in items if isinstance(i, dict)],
            key=lambda i: float(i.get("created_at") or 0),
            reverse=True,
        )
    ...
    def list_plugins() -> dict[str, Any]:
        ...
        items = [
            {
                **p,
                "enabled": bool(state.get(p["id"], True))
                if p.get("available")
                else False,
            }
            for p in PLUGIN_CATALOG
    ...
    def toggle_plugin(request: PluginToggleRequest) -> dict[str, Any]:
        ...
        return {
            ...
            "enabled": state[request.plugin_id],
        }
