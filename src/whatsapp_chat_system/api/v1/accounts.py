...
def create_accounts_router(
    ...
    def create_account(request: Request, payload: AccountCreateRequest,
    ...
    def update_account(account_id: str, request: Request, payload: AccountUpdateRequest,
                       session: Session = Depends(get_session)) -> Any:
        service = AccountService(session)
        changes = payload.model_dump(exclude_unset=True)
        if 'name' in changes and changes['name'] is None:
            return error_response(request, 'validation_error', 'name must not be null', status_code=422)
        try:
    ...
    def connect_account(account_id: str, request: Request,
                        session: Session = Depends(get_session)) -> Any:
        service = AccountService(session)
        try:
    ...
    def account_qr(account_id: str, request: Request,
                   session: Session = Depends(get_session)) -> Any:
        service = AccountService(session)
        try:
            account = service.get(account_id)
            if account.status != 'qr_pending':
                return error_response(request, 'qr_not_pending',
    ...
    def logout_account(account_id: str, request: Request,
                       session: Session = Depends(get_session)) -> Any:
        service = AccountService(session)
        try:
    ...
    def delete_account(account_id: str, request: Request, payload: AccountDeleteRequest,
                       session: Session = Depends(get_session)) -> Any:
        service = AccountService(session)
        try:
            account = service.get(account_id)
