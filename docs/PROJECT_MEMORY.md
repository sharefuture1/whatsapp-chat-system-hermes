...
- 2026-09-11：**`whats.wending.ai` 前后端已统一部署到 gcptw**。Nginx 直接服务 `/opt/whatsapp-chat-system/web/dist` 的 React SPA，根路径与深链均 200；浏览器使用同源相对 `/api`，不依赖 Vercel。FastAPI `127.0.0.1:8792` 与 Bridge V2 `127.0.0.1:3100` 均由专用低权限账户运行；SQLite 位于 Alembic `0005 (head)`，API `/api/health`、`/health/live`、`/health/ready` 与 Bridge live/ready 均通过。公网 `/internal/*` 继续 404，SSE 路径关闭 buffering，Let's Encrypt/Cloudflare TLS 正常。认证已迁移为正式 `admin` 用户，旧 session 清空。Vercel 保留为可选备用部署，不再是当前生产前端依赖。真实 WhatsApp 扫码、收发与 24h 自动回复仍需业务账号在线后单独验收。
...
- 前端：`index-BqseTvKJ.js` / `index-CyWC_Vl7.css`，本机 FastAPI 与公网 `https://whats.future1.us` 资源哈希一致。
