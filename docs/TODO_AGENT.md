...
- [>] **统一前端数据层与性能预算**
  ...
  - [ ] cursor/SSE 增量同步
  - [>] 翻译数据库批处理与管理员策略（已落地 DB 真源 Phase 1：`message_translations` / `translation_batches` + batch enqueue API + 进程内 TranslationDispatcher；ChatPane 已优先走 batch API）
  - [ ] 媒体代理、AI Job、可观测性
