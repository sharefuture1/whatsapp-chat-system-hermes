...
- 状态：`In Progress`
- 本轮进展：聊天、通讯录、发现、我、账号中心已统一使用独立页面壳、滚动容器和移动端安全区；全局 AI 配置保持在“我”页，聊天页仅显示当前联系人上下文。
- 剩余内容：
  - “发现”改“工作台”；
  - “我”页进一步只保留操作员和全局设置；
  - 聊天头部只保留返回、名称、状态和“…”；
...
- 状态：`Implemented`
- 已实现：插件开关、消息设置和 AI Provider 形成统一有效门禁；运行时加密配置热生效；Legacy 整数 ID 与 V2 UUID 均可翻译；失败向 UI 返回可操作错误，不再静默或把原文伪装成译文。
