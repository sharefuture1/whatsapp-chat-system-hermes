...
assert(!/^import .*@tauri-apps\/plugin-http/m.test(apiClient), 'Browser entry must not statically import the Tauri HTTP plugin')
assert(apiClient.includes("import('@tauri-apps/plugin-http')"), 'Web API client must lazy-load the Tauri HTTP plugin')
